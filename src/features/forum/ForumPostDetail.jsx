...
const ForumPostDetail = () => {
  ...
  const handleSaveEditComment = async (commentId) => {
    try {
      await api.put(`/forum/posts/${id}/comments/${commentId}`, { content: editCommentContent });
    ...
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-6">
      ...
      <Card className="p-8">
        ...
        <div className="mb-6">
          ...
          <span className="inline-block px-3 py-1 bg-slate-800 text-slate-400 rounded-full text-xs font-medium">{post.category}</span>
        </div>
        ...
        <div className="flex items-center gap-4 border-t border-slate-800 pt-4 mb-8">
          <button
            ...
            className={`flex items-center gap-2 text-sm transition-colors ${post.upvotes.includes(user._id) ? 'text-primary' : 'text-slate-400 hover:text-primary'}`}
          >
            <ThumbsUp size={18} />
            {post.upvotes.length}
          </button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      ...
                      {comment.updatedAt > comment.createdAt && <span className="ml-2">(edited)</span>}
                    </p>
                  </div>
                </div>
                {comment.authorId?._id === user._id && (
                  <div className="flex gap-2">
                    <button onClick={() => handleEditComment(comment._id)} className="text-slate-400 hover:text-primary">
                      <Edit size={16} />
                    </button>
                    <button onClick={() => handleDeleteComment(comment._id)} className="text-slate-400 hover:text-red-400">
                      <Trash2 size={16} />
                    </button>
                  </div>
                )}
              ...
              {editingCommentId === comment._id ? (
                <div className="flex gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <Button variant="ghost" onClick={() => setEditingCommentId(null)} size="small">
                      Cancel
                    </Button>
                  </div>
                </div>
